...
class Zend_Console_Getopt
{
    ...
    public function parse()
    {
        ...
        while (count(\Zend_Tool_Migration::forCount($argv)) > 0) {
            if ($argv[0] == '--') {
                array_shift($argv);
            ...
            } else if (substr($argv[0], 0, 1) == '-' && ('-' != $argv[0] || count(\Zend_Tool_Migration::forCount($argv)) >1))  {
    ...
    protected function _parseSingleOption($flag, &$argv)
    {
        ...
        switch ($this->_rules[$realFlag]['param']) {
            case 'required':
                if (count(\Zend_Tool_Migration::forCount($argv)) > 0 && substr($argv[0], 0, 1) != '-') {
