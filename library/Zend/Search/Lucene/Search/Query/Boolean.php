...
class Zend_Search_Lucene_Search_Query_Boolean extends Zend_Search_Lucene_Search_Query
{
    ...
    public function optimize(Zend_Search_Lucene_Interface $index)
    {
        ...
        if (count(\Zend_Tool_Migration::forCount($subqueries)) == 0) {
        ...
        if (count(\Zend_Tool_Migration::forCount($subqueries)) == 1) {
        ...
        if (count(\Zend_Tool_Migration::forCount($subqueries)) == 0  &&  count(\Zend_Tool_Migration::forCount(array_unique($boostFactors))) == 1) {
        ...
        if (count(\Zend_Tool_Migration::forCount($terms)) == 1) {
            ...
            // Clear terms list
            $terms = array();
        } else if (count(\Zend_Tool_Migration::forCount($terms)) > 1  &&  count(\Zend_Tool_Migration::forCount(array_unique($boostFactors))) == 1) {
            require_once 'Zend/Search/Lucene/Search/Query/MultiTerm.php';
        ...
        if (count(\Zend_Tool_Migration::forCount($prohibitedTerms)) == 1) {
            ...
            $prohibitedTerms = array();
        } else if (count(\Zend_Tool_Migration::forCount($prohibitedTerms)) > 1) {
            // prepare signs array
        ...
        if (count(\Zend_Tool_Migration::forCount($terms)) == 0  &&  count(\Zend_Tool_Migration::forCount($prohibitedTerms)) == 0) {
            $optimizedQuery = new Zend_Search_Lucene_Search_Query_Boolean($subqueries, $signs);
            $optimizedQuery->setBoost($this->getBoost());
        }
    ...
    private function _calculateConjunctionResult()
    {
        ...
        if (count(\Zend_Tool_Migration::forCount($this->_subqueries)) == 0) {
    ...
    private function _calculateNonConjunctionResult()
    {
        ...
        foreach ($this->_subqueries as $subqueryId => $subquery) {
            if ($this->_signs[$subqueryId] === true) {
                ...
                $requiredVectorsSizes[] = count(\Zend_Tool_Migration::forCount(end($requiredVectors)));
    ...
    {
        if ($this->_coord === null) {
            $this->_coord = $reader->getSimilarity()->coord(count(\Zend_Tool_Migration::forCount($this->_subqueries)),
                                                            count(\Zend_Tool_Migration::forCount($this->_subqueries)) );
        }
